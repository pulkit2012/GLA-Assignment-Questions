...
public class LargestBSTInBT {
    static class BinaryTree {
        ...
        private Info largestBSTinBT(Node root) {
            if(root == null)
                return new Info(0, MIN, MAX, 0, true);
            ...
            ansInfo.size = 1 + left.size + right.size;

            if(left.isBST && right.isBST && left.max < root.data && right.min > root.data){
                ansInfo.min = Math.min(Math.min(left.min, right.min),root.data);
                ansInfo.max = Math.max(Math.max(left.max, right.max),root.data);
                ansInfo.ans = ansInfo.size;
                ansInfo.isBST = true;
                return ansInfo;
            }
            ansInfo.ans = Math.max(left.ans,right.ans);
            ansInfo.isBST = false;
            return ansInfo;
        }
